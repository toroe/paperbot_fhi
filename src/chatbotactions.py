...
class ChatBot:
    def __init__(self, token, primary_channel_id):
        ...
        token = "cw4ona15y3f57mobs3p6rdba5r"
        logging.basicConfig(
        ...
        self.driver = Driver(options={"url": "localhost", "token":token,"scheme":"http"})
        self.driver.login()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--channel_id", default="u7km46zfofrpzf3q7m344mj5kw", help="Channel ID article should be posted in")
    parser.add_argument("--db_connection", default="bolt://localhost:7687")
    args = parser.parse_args()
    db_driver = GraphDBDriver(args.db_connection, os.environ["NEO4J_USER"],"$cheisse4ldder")
    bot = ChatBot(token = "cw4ona15y3f57mobs3p6rdba5r",primary_channel_id=args.channel_id)
